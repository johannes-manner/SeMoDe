...
public final class AWSLogAnalyzer {
     ...
     * @return
     */
    public static FunctionInstrumentation generateFunctionInstrumentation(final FunctionExecutionEvent logEvent) {
        String handlerClass = "";
        ...
        for (final OutputLogEvent event : logEvent.getEvents()) {
            if (event.getMessage().startsWith(TROUBLESHOOT_PREFIX)) {
                final String[] splitResult = event.getMessage().split(TROUBLESHOOT_SPLIT_PATTERN);
                if (splitResult.length == 4) {
                    final String key = splitResult[2];
                    final String value = splitResult[3];
